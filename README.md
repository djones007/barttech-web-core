...
| `uploads.ts` | `UPLOAD_LIMITS`, `IMAGE_MIME_TYPES`, `DOCUMENT_MIME_TYPES`, `sniffMimeType`, `safeUploadFilename` (alias `sanitizeStorageSegment`), `validateUpload` — server-side upload validation. Never trusts the client-declared MIME type or the extension; sniffs magic bytes. Sanitises filenames for storage keys (path-traversal guard) — the single sanitiser every `.storage.from(...)` write/read-by-key call must reuse, enforced by `scripts/check-storage-path-traversal.mjs`. |
