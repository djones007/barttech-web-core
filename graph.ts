...
//
// WHY THIS EXISTS
// Six routes each hand-rolled the same OAuth-token-then-sendMail dance, and
// five of them had NO retry of any kind. That is the exact failure that lost a
// customer's quote email on 2026-07-29: a single transient rejection, one
// dropped message, no error surfaced to anyone. When these sends fail, Dom
// simply never hears that a lead came in — there is no bounce, no alert, and
// the visitor still sees a success screen. Silence is the failure mode, which
// is why the retry belongs here rather than in whichever route remembers it.
//
// WHAT IS RETRIED, AND WHAT IS NOT
//   * fetch() itself throwing — Graph intermittently resets the TLS socket
//     mid-handshake. Retried (this is barttech-website's `fetchWithRetry`,
//     which was the only correct handling of it in the estate).
...
// across the estate — `GRAPH_*`, `MS_GRAPH_*` and `MS_*`. All three are read,
// in that order. This is deliberate: renaming them would mean editing six
// Vercel projects for zero functional gain, and a missed one silently stops
// that brand's notifications. Set whichever the repo already uses.
//
...
/** The one sending mailbox for the estate. Notifications are sent as, and by default to, this address. */
export const GRAPH_MAILBOX = "dominicj@barttech.co.uk";
...
/**
 * Create an Outlook **draft** (`POST /messages`). Never sends. Used for the
 * Cultrix onboarding handoff, which Dom reviews and sends himself — do not
 * "improve" this into a sendMail call.
 */
