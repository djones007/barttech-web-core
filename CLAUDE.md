...
1d. **One sanitiser engine, two names (consolidated 2026-08-15).** `renderSafeHtml` is an alias of `renderSafeHtmlNoDom` — the sanitize-html implementation in `safeHtmlNoDom.ts`. The DOMPurify/jsdom engine is retired: rule 1c's module-load crash made it unusable in any serverless/edge runtime, and the consolidation was done the way the previous version of this rule required — every consumer's full published corpus rendered through both engines and DOM-diffed (tags, every attribute, text content, JSON-LD block data) before the alias shipped. Both names stay exported (golden rule 4). Do not reintroduce jsdom or any transitive dependency on it; if a second engine ever returns, the corpus-diff obligation applies again — `safeHtmlNoDom.test.ts` pins the single-implementation invariant so that reintroduction fails a test rather than passing silently.
