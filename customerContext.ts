...
// SERVER-SIDE ONLY. Uses the the checkout app service-role key and a Shopify
// admin token — never import this into a client component.
//
// Promoted out of `repos/a private app/src/lib/support/customer-context.ts` on
...
/**
 * Shopify orders for a client brand, including tracking numbers.
 *
 ...
 *
 * a client brand only — it is the one brand with a Shopify storefront. Called
...
async function getShopifyOrders(email: string): Promise<{ orders: ShopifyOrder[]; error?: string }> {
  ...
  const shop = process.env.SHOPIFY_STORE_SHOP; // e.g. example.myshopify.com
 ...
 *
 * `brandSlug` gates the Shopify lookup — only a client brand has a storefront, and
 * querying it for an a client brand ticket would be a wasted round trip on every
 * page load.
 */
