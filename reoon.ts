...
//
// WHY THIS EXISTS
// Before 2026-07-29 three consumer routes asked that question and gave three
// different answers:
//
//   consumer A (public site)      blocklist: invalid, disposable, unsafe
//   consumer B (public site)      blocklist: invalid, disposable, unsafe
//   consumer C (B2B contact form) allowlist: only `safe`/`valid` reach the ESP
//
// So a **spamtrap** address was accepted outright by two of the three. Spamtraps
// are the single most damaging thing to put on a list — they exist to catch
// senders who don't clean their data — and one of those brands was already
// dealing with a mailbox-provider IP-pool demotion. The blocked set below is
 ...
 * to hold back anything unproven. One B2B contact route works this way
 ...
 * looks, never what we do with the answer. That same B2B contact form uses
 * `power` because its leads are individually worth the credits; the
...
//
// The internal CDP's own bulk-suppression verifier also talks to Reoon, and was
// deliberately NOT folded in here. It answers a different question — "should we
// STOP emailing a contact we already have?" — using the bulk task API (cheaper per address,
...
// unknown and role accounts (a generic info@ address is a normal B2B optin
// for at least one consuming brand).
//
